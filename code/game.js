...
var actorChars = {
  ...
  "*": End // A coin that goes to the next level when acquired
};
...
End.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
/* Level.prototype.playerTouched = function(type, actor) {				//TEST  - Did not grab both coins, either grabbed coin or end, but not both
  if (type == "end") {
